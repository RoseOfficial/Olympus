...
public sealed class BuffModule : INikeModule
{
    ...
    private bool TryShoha(INikeContext context, IBattleChara target)
    {
        ...
        if (context.ActionService.ExecuteOgcd(SAMActions.Shoha, target.GameObjectId))
        {
            ...
            MeleeDpsTrainingHelper.RecordResourceDecision(
                ...
                SAMActions.Shoha.ActionId,
                SAMActions.Shoha.Name,
                "Meditation",
                context.Meditation,
                $"Spending {MeditationMaxStacks} Meditation on Shoha",
                ...
                new[] { "3 Meditation stacks", "oGCD window available", "High potency damage" },
                new[] { "Wait for burst (risk overcapping)", "Let stacks expire (wastes damage)" },
                "Shoha is free damage. Use it whenever you have 3 Meditation stacks.",
                "sam_meditation");
    ...
    private bool TryZanshin(INikeContext context, IBattleChara target)
    {
        ...
        if (context.ActionService.ExecuteOgcd(SAMActions.Zanshin, target.GameObjectId))
        {
            ...
            MeleeDpsTrainingHelper.RecordDamageDecision(
                context.TrainingService,
                SAMActions.Zanshin.ActionId,
                SAMActions.Zanshin.Name,
                target.Name?.TextValue ?? "Target",
                "Using Zanshin proc (from Ogi Namikiri)",
                ...
                new[] { "Zanshin Ready proc", $"Kenki >= 50 ({context.Kenki})", "Burst window" },
                new[] { "Delay for other oGCDs (loses proc time)", "Save Kenki (loses burst damage)" },
                "Zanshin is part of your burst combo: Ikishoten → Ogi Namikiri → Kaeshi → Zanshin.",
                "sam_zanshin");
    ...
    private bool TryIkishoten(INikeContext context)
    {
        ...
        if (context.ActionService.ExecuteOgcd(SAMActions.Ikishoten, player.GameObjectId))
        {
            ...
            MeleeDpsTrainingHelper.RecordBurstDecision(
                context.TrainingService,
                SAMActions.Ikishoten.ActionId,
                SAMActions.Ikishoten.Name,
                "Self",
                "Activating Ikishoten burst window (+50 Kenki + Ogi Namikiri Ready)",
                ...
                new[] { "120s cooldown ready", $"Kenki low enough ({context.Kenki})", "No Ogi Namikiri Ready", "Burst window" },
                new[] { "Wait for raid buffs (if imminent)", "Use at high Kenki (wastes gauge)" },
                "Time Ikishoten with raid buffs. Don't overcap Kenki - use before if at 50+.",
                "sam_ikishoten");
    ...
    private bool TryMeikyoShisui(INikeContext context)
    {
        ...
        if (context.ActionService.ExecuteOgcd(SAMActions.MeikyoShisui, player.GameObjectId))
        {
            ...
            MeleeDpsTrainingHelper.RecordBurstDecision(
                context.TrainingService,
                SAMActions.MeikyoShisui.ActionId,
                SAMActions.MeikyoShisui.Name,
                "Self",
                $"Activating Meikyo Shisui ({reason})",
                ...
                new[] { reason, "55s cooldown ready (2 charges)", $"Sen: {context.SenCount}/3" },
                new[] { "Wait for optimal timing (risk overcapping charges)", "Use during combos (less efficient)" },
                "Meikyo → Gekko → Kasha → Yukikaze builds all 3 Sen instantly.",
                "sam_meikyo_shisui");
    ...
    private bool TryBurstKenki(INikeContext context, IBattleChara target)
    {
        ...
        if (useAoe)
        {
            ...
            if (context.ActionService.ExecuteOgcd(SAMActions.Guren, player.GameObjectId))
            {
                ...
                MeleeDpsTrainingHelper.RecordAoeDecision(
                    context.TrainingService,
                    SAMActions.Guren.ActionId,
                    SAMActions.Guren.Name,
                    enemyCount,
                    $"Spending 25 Kenki on Guren AoE ({enemyCount} enemies)",
                    "Guren is the AoE version of Senei, dealing line damage. " +
                    "Use at 3+ enemies for better total potency than Senei.",
                    new[] { "Kenki >= 25", $"{enemyCount} enemies", "120s cooldown ready" },
                    new[] { "Use Senei (less total damage vs 3+)", "Save Kenki (if Ikishoten soon)" },
                    "Guren vs Senei: Guren wins at 3+ enemies, Senei for single target.",
                    "sam_kenki_gauge");
        ...
        else if (level >= SAMActions.Senei.MinLevel)
        {
            ...
            if (context.ActionService.ExecuteOgcd(SAMActions.Senei, target.GameObjectId))
            {
                ...
                MeleeDpsTrainingHelper.RecordDamageDecision(
                    context.TrainingService,
                    SAMActions.Senei.ActionId,
                    SAMActions.Senei.Name,
                    target.Name?.TextValue ?? "Target",
                    "Spending 25 Kenki on Senei burst",
                    ...
                    new[] { "Kenki >= 25", "120s cooldown ready", "Single target priority" },
                    new[] { "Use Guren (only if 3+ enemies)", "Save for later (if burst imminent)" },
                    "Senei deals massive single-target damage. Align with raid buffs.",
                    "sam_kenki_gauge");
