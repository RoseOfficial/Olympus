...
/// <summary>
/// Handles the Warrior enmity management.
...
public sealed class EnmityModule : IAresModule
{
    ...
    private bool TryProvoke(IAresContext context)
    {
        var player = context.Player;
        var level = player.Level;

        if (level < WARActions.Provoke.MinLevel)
            return false;
        ...
        if (pendingSwap != null && !pendingSwap.IntendToTakeAggro)
        {
            ...
            if (!context.ActionService.IsActionReady(WARActions.Provoke.ActionId))
            {
            ...
            if (context.ActionService.ExecuteOgcd(WARActions.Provoke, target.GameObjectId))
            {
                ...
                TrainingHelper.Decision(context.TrainingService)
                    .Action(WARActions.Provoke.ActionId, WARActions.Provoke.Name)
                    .AsEnmity()
    ...
    private bool TryShirk(IAresContext context)
    {
        var player = context.Player;
        var level = player.Level;

        if (level < WARActions.Shirk.MinLevel)
            return false;
        ...
        if (pendingSwap != null && pendingSwap.IntendToTakeAggro)
        {
            ...
            if (!context.ActionService.IsActionReady(WARActions.Shirk.ActionId))
            {
            ...
            if (context.ActionService.ExecuteOgcd(WARActions.Shirk, coTankForSwap.GameObjectId))
            {
                partyCoord?.ClearTankSwapReservation(targetEntityId);
                ...
                TrainingHelper.Decision(context.TrainingService)
                    .Action(WARActions.Shirk.ActionId, WARActions.Shirk.Name)
                    .AsEnmity()
        ...
        // Execute Shirk
        if (context.ActionService.ExecuteOgcd(WARActions.Shirk, coTank.GameObjectId))
        {
            context.Debug.PlannedAction = WARActions.Shirk.Name;
