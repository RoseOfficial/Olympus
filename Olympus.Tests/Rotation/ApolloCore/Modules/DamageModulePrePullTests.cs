...
/// <summary>
/// Tests for the WHM DamageModule pre-pull hardcast branch.
/// At level 90, GetDamageGcdForLevel returns Glare III (ActionId=25859, cast 1.5s,
...
public class DamageModulePrePullTests
{
    ...
    [Fact]
    public void PrePullHardcast_WithinThreshold_PushesGlareIIIAtPriority5()
    {
        ...
        Assert.Contains(gcd, c => c.Behavior.Action.ActionId == WHMActions.GlareIII.ActionId && c.Priority == 5);
    }
    ...
    [Fact]
    public void PrePullHardcast_NoTarget_DoesNotPushGlareIII()
    {
        var context = ApolloTestContext.Create(
            level: 90,
            inCombat: false,
    ...
    [Fact]
    public void PrePullHardcast_ToggleOff_DoesNotPushGlareIII()
    {
        var enemy = CreateMockEnemy();
        var targeting = MockBuilders.CreateMockTargetingService();
        targeting.Setup(x => x.GetUserEnemyTarget()).Returns(enemy.Object);
