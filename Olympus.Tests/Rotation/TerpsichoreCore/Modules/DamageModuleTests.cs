...
public class DamageModuleTests
{
    ...
    /// All DNC GCDs have CastTime=0 so the gate is a practical no-op at cap,
    ...
    public void CastTimeGateWired_Smoke_FillerFiresEvenWithImminentRaidwide()
    {
        ...
        // All DNC GCDs are instant (CastTime=0), so the gate does NOT block.
        // Filler path (Cascade) should still execute.
        var context = TerpsichoreTestContext.Create(
            config: config,
            inCombat: true,
            canExecuteGcd: true,
