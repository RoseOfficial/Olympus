...
public class DamageModuleTests
{
    ...
    /// All BRD GCDs have CastTime=0 so the gate is a practical no-op at cap,
    ...
    public void CastTimeGateWired_Smoke_FillerFiresEvenWithImminentRaidwide()
    {
        ...
        // All BRD GCDs are instant (CastTime=0), so the gate does NOT block.
        // Filler path (BurstShot) should still execute.
        var context = CalliopeTestContext.Create(
            config: config,
            inCombat: true,
            canExecuteGcd: true,
